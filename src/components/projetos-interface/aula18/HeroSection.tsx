...
import { Figma, Layout, MousePointer, Palette } from "lucide-react";
...
const HeroSection = () => {
  return (
    <section id="hero" className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_center,_var(--tw-gradient-stops))] from-violet-500/10 via-transparent to-transparent" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-24 text-center">
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-6">
            Introdução ao{" "}
            <span className="text-violet-600 dark:text-violet-400">Figma</span>
          </h1>

          <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-12">
            Aprenda os fundamentos da ferramenta mais popular para 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {[
            { icon: Layout, label: "Frames e Layout", delay: 0.2 },
            { icon: MousePointer, label: "Ferramentas Básicas", delay: 0.3 },
            { icon: Palette, label: "Estilos e Cores", delay: 0.4 },
          ].map((item, index) => (
            <motion.div
              key={index}
