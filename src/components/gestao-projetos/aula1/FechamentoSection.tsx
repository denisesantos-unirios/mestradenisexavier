...
import { Award, Key, AlertTriangle, Lightbulb, BookOpen, MessageSquare } from "lucide-react";
    ...
    <section id="fechamento" className="py-20 px-6 relative overflow-hidden bg-secondary/20">
      <div className="max-w-5xl mx-auto relative z-10">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {["Sprint", "PO", "SM", "Daily", "Review", "Retrospective", "Backlog", "Incremento", "Auto-organização", "Time-box"].map((w, i) => (
              ...
              <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {["Fluxo contínuo", "Quadro", "Cartões", "WIP", "Lead Time", "Cycle Time", "Throughput", "Gargalo", "Pull system"].map((w, i) => (
                    <span key={i} className="px-2 py-1 rounded bg-emerald-500/10 text-xs text-emerald-300">{w}</span>
                  ))}
                </div>
              </div>
              <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4">
                <h4 className="font-bold text-purple-400 mb-2">⚡ Ágil</h4>
                <div className="flex flex-wrap gap-1.5">
                  {["Iterativo", "Incremental", "Feedback", "Adaptação", "Valor de negócio", "MVP", "Manifesto", "Colaboração"].map((w, i) => (
          ...
          <motion.div whileHover={{ scale: 1.01 }} className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-6 mb-8">
            ...
            <div className="space-y-4">
              {[
                ...
                {
                  tip: "Elimine os absurdos primeiro",
                  detail: "Alternativas com \"não usar nenhuma metodologia\" ou \"gerente controla tudo\" são quase sempre erradas no contexto ágil."
                }
              ].map((item, i) => (
        ...
        <ScrollReveal animation="scale" delay={0.3}>
          <motion.div whileHover={{ scale: 1.01 }} className="bg-primary/10 border border-primary/30 rounded-2xl p-8">
        ...
        <ScrollReveal animation="fadeUp" delay={0.4}>
          <div className="mt-10 grid md:grid-cols-3 gap-4">
            <motion.div whileHover={{ y: -5 }} className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-5 text-center">
              ...
              <p className="text-sm text-muted-foreground">Produto novo, escopo variável, time dedicado, Sprints fixas</p>
            </motion.div>
            <motion.div whileHover={{ y: -5 }} className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-5 text-center">
              ...
              <p className="text-sm text-muted-foreground">Suporte, manutenção, fluxo contínuo, demandas imprevisíveis</p>
            </motion.div>
            <motion.div whileHover={{ y: -5 }} className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-5 text-center">
              <p className="text-3xl mb-2">🔀</p>
              <h4 className="font-bold text-foreground mb-1">Híbrido</h4>
