...
import { Boxes, Workflow, GitBranch } from "lucide-react";
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-24 text-center">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-violet-500/10 text-violet-400 text-sm font-medium mb-6">
            ...
            Semana 7 • Aula Presencial 1
          </span>

          <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-6">
            Introdução à{" "}
            <span className="text-violet-400">UML</span>
          </h1>

          <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-12">
            Unified Modeling Language - A linguagem padrão para visualização, 
            especificação e documentação de sistemas orientados a objetos.
          </p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {[
            { icon: Boxes, label: "Diagramas Estruturais", delay: 0.2 },
            { icon: Workflow, label: "Diagramas Comportamentais", delay: 0.3 },
            { icon: GitBranch, label: "Modelagem Visual", delay: 0.4 },
          ].map((item, index) => (
            <motion.div
              key={index}
