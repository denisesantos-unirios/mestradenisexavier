      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-24 text-center">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 text-amber-500 text-sm font-medium mb-6">
            ...
            Semana 6 • Aula Virtual (JITT)
          </span>

          <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-6">
            Fórum: Entrega{" "}
            <span className="text-amber-500">Fase 2</span>
          </h1>

          <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-12">
            Preparação e entrega do Documento de Requisitos completo 
            para avaliação da Fase 2 do Projeto Integrador.
          </p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {[
            { icon: FileText, label: "Documento RF/RNF", delay: 0.2 },
            { icon: Upload, label: "Entrega no AVA", delay: 0.3 },
            { icon: MessageCircle, label: "Discussão no Fórum", delay: 0.4 },
          ].map((item, index) => (
            <motion.div
              key={index}
